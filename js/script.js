...
WOLF_PATTERN_WIDTH  = 80 *  WOLF_SCALE; // Note: Width is likely 80 based on the highest X value in the path data
WOLF_PATTERN_HEIGHT = 110 * WOLF_SCALE; // Height is 110 based on the highest Y value
...
// 1. PLACE WOLF STARS FIRST (They define the shape)
...
for (let i = 0; i  < STAR_COUNT; i++) {
    for (let attempt = 0; attempt  < MAX_ATTEMPTS; attempt++) {
        ...
        // A. Check against Wolf Stars (joints)  — use tighter buffer to preserve joints
...
svg.addEventListener('click', e => {
    ...
    const clickBuffer = 15; // Pixels of tolerance
    
    // Check if click is near any wolf star
    for (const star of wolfStarData) {
        if (Math.hypot(e.clientX - star.x, e.clientY - star.y) < clickBuffer) {
            revealWolf();
            return; // Exit after finding the first match
        }
    }

    // Check if click is near any wolf line
    for (const [i, j] of WOLF_CONNECTIONS) {
        const s1 = wolfStarData[i];
        const s2 = wolfStarData[j];
        if (pointToSegmentDistance(e.clientX, e.clientY, s1.x, s1.y, s2.x, s2.y) < clickBuffer) {
            revealWolf();
            return; // Exit after finding the first match
        }
    }
...
svg.setAttribute('width',  window.innerWidth);
svg.setAttribute('height', window.innerHeight);
placeStars(); // Re-run placement logic on resize
initDOM();    // Re-render the DOM elements
});
init();
